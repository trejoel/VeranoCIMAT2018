...
set <int, greater <int> > TableConflict;
...
public:
    ...
    inConflict=(vertex*) malloc (sizeof(vertex)*V); //This is the list of vertices in conflict
	  howManyConflict=0; //Initially there exists zero vertices in conflict
  	colorGraph(V); 
		//printColoring(V);
    coloringToFile(V);
 ...
 ////create vertex
  ...
  void colorGraph(int size){
    ...
    for (int i=0;i<size;i++){
  	...
  	// shared (inConflict) reduction(+:howManyConflict)
    #pragma omp parallel for
    for (int i=0;i<size;i++){
      #pragma omp critical
      existsConflict(i);
    }
    colorConflictVertices(TableConflict);
    wtime = omp_get_wtime ()-wtime;
    printf("Tiempo de respuesta secuencial:%f\n",wtime);

    //printSetConflictVertices(TableConflict);
  }

   void existsConflict(int pos){
     vertex v=vertice[pos];
     //cout<<"vertex:"<<v.id<<endl;
     int vColor=vertice[pos].color;
     int colorNeighbor,idNeighbor;
     int j,z;
     z=vertice[pos].degree;
     for (j=0;j<z;j++){
       if (vColor==vertice[v.neighborhood[j]].color && v.id < v.neighborhood[j]) {
        TableConflict.insert(v.id);
       }

     }
   }

  void colorConflictVertices(set <int, greater <int> > conflicting){
       int vertexID;
       set <int, greater <int> > :: iterator itr;
       for (itr = conflicting.begin(); itr != conflicting.end(); ++itr){
        vertexID=*itr;
        setColor(vertexID);
        //cout << "Conflicting vertex:" << *itr<<endl;
    }
  }

  void printSetConflictVertices(set <int, greater <int> > conflicting){
       set <int, greater <int> > :: iterator itr;
       for (itr = conflicting.begin(); itr != conflicting.end(); ++itr){
        cout << "Conflicting vertex:" << *itr<<endl;
    }
